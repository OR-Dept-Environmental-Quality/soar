...
from soar.loaders.filesystem import append_csv
...
def write_annual_for_parameter(parameter_code: str, analyte_name: str, bdate: date, edate: date, state_fips: str, session=None) -> dict:
    session = session or _client.make_session()
    safe_name = _sanitize_filename(analyte_name)
    results = {"parameter": parameter_code, "analyte_name": analyte_name, "years": {}, "status": "ok"}
    ...
    try:
        for year_token, df in fetch_annual_by_state(parameter_code, bdate, edate, state_fips, session=session):
            ...
            out_path = year_dir / f"aqs_annual_{safe_name}.csv"
            if df is None or df.empty:
    ...
    audit_name = f"annual_{parameter_code}_{safe_name}_{ts}.json"

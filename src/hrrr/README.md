...
Extracts hourly mixing height (Planetary Boundry Layer Height) from NOAA's High Resolution Rapid Refresh Model (HRRR) for Oregon moinitoritng sites, and combines it with AQS wind speed data to compute ventilation index.
...
- 'src/stage/consolidate_mixing_height.py' + 'pipelines/hrrr/fun_fct_mixing_height.py'
...
### 1. Pull raw HRRR data
    ...
    - **Keep raw GRIB2 files?** -- 'y' keeps the downloaded grid files in 'raw/hrrr_grib' (~25 GB/year); 'n' (default) deletes each file after the required values are extratced 
...
### 2. Stage the yearly mixing height files
    ...
    'staged/fct_mixing_height_{year}.csv'. Prints a warning is a year's daily file count doesn't match the expected number of days for that year. 
...
- **Downloads use byte-range requests** not full HRRR files. NOAA publishes an '.idx' index alongside each hourly grid file; this pipeline reads that index to find the HPBL feild's byte range and downloads only that slice. This cuts the extraction from ~100 MB to ~3 MB per hour. 
